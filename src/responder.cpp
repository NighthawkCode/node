...
static void responder_thread_proc() {
  ...
  while (!responder_quit) {
    ...
    if (pollresult == -1) {
      ...
      if (pfd.revents & POLLIN) {
        ...
        if ((recvStringLen = recvfrom(sock, recvString, MAXRECVSTRING, 0, (sockaddr*)&Sender_addr,
                                      (socklen_t*)&len)) < 0) {
          ...
          if (sendto(sock, respondmessage, strlen(respondmessage) + 1, 0, (sockaddr*)&Sender_addr,
